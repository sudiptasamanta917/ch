...
const createUniqueUrls = (noOfPlayers, gameTime, tournamentId, roundId) => {
  ...
  // const host = "dynamo-unicorn-chess.vercel.app";
  const urls = [];
    ...
    for (let i = 0; i < scoreGroups.length; i++) {
      ...
      if (totalPlayers % 2 !== 0) {
        ...
        for (let j = i + 1; j < scoreGroups.length; j++) {
          ...
          console.log(
            nextGroup,
            "++++odd wala hai upar se niche nextGroup ko bhi dekho ++++++++++++"
          );
          console.log(
            `Player ${lastPlayer.user} moved to score group ${j} nicheeeeeeeeeeee`
          );
          moved = true;
          break;
        }
        ...
        if (!moved) {
          for (let j = i - 1; j >= 0; j--) {
            ...
            console.log(
              nextGroup,
              "++++odd wala niche se upar nextGroup ko bhi dekho ++++++++++++"
            );
            console.log(
              `Player ${lastPlayer.user} moved to score group ${j} uparrrrrrrr`
            );
            moved = true;
            break;
          }
        ...
        if (!moved) {
          ...
          console.log(
            `Player ${lastPlayer.user} moved to a new score group. nayaaaaaaaaaaaa`
          );
        }
      }
      ...
      console.log(
        group,
        group.length,
        "-----------------kahuaaaaaa reeeee dekhaaaa grioup ko ------------------------"
      );
      ...
      for (let k = 0; k < midIndex; k++) {
        ...
        if (previousMatch) {
          ...
          for (let j = i + 1; j < scoreGroups.length; j++) {
            ...
            console.log(
              `Players ${player1.user} and ${player2.user} moved to score group ${j}`
            );
            moved = true;
            break;
          }
          ...
          if (!moved) {
            ...
            console.log(
              `Players ${player1.user} and ${player2.user} moved to a new score group.`
            );
            // Remove player2 first (to prevent index shifting)
            group.splice(player2Index, 1);
            // Remove player1 second
  ...
  for (let i = 0; i < scoreGroups.length; i++) {
    ...
    for (let j = 0; j < half; j++) {
      ...
      if (!previousMatch) {
        ...
        const haveSameLastThreeColors = (player1, player2) => {
          const player1Colors = player1.colorHistory.slice(-3); // Get last three colors of player1
          const player2Colors = player2.colorHistory.slice(-3); // Get last three colors of player2

          // Check if the last three colors are identical for both players
          return (
            player1Colors.length === 3 &&
            player2Colors.length === 3 &&
            player1Colors[0] === player2Colors[0] &&
            player1Colors[1] === player2Colors[1] &&
            player1Colors[2] === player2Colors[2]
          );
        };
        // Perform the color check
        if (haveSameLastThreeColors(player1, player2)) {
          console.log(
            `Skipping match creation for ${player1.user} and ${player2.user} due to same last three colors.`
          );
          ...
          if (i + 1 < scoreGroups.length) {
            ...
            console.log(
              `No next group exists for ${player1.user} and ${player2.user}`
            );
          }
          continue; // Skip the current iteration if colors match
        } else {
          const match = await Match.create({
            round: round._id,
            tournamentId: tournamentId,
    ...
    if (scoreGroups.length > 0) {
      console.log(
        scoreGroups,
        scoreGroups.length,
        "989898989898888888888888888888888888888888888888888888888888888888"
      );
      ...
      for (let i = 0; i < scoreGroups.length; i++) {
        ...
        console.log(
          ...
          "adadaddadadadadddddddddddddaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        );
        ...
        for (let j = 0; j < leftoverPlayers.length; j++) {
          ...
          for (k = j + 1; k < leftoverPlayers.length; k++) {
            ...
            if (
              ...
              console.log(
                "+++++usedddddddddddddddddddddddddddddddddddplayers hai++++++++++"
              );
            ...
            if (previousMatch) {
              console.log(
                "previous match mai thaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
              );
              continue;
            } else {
              const matchUrl = urls[urlIndex++ % urls.length];
    ...
    if (roundNumber == 2) {
      if (tournament.status == "ongoing") {
        ...
        if (tournament.JoinedPlayerList.length < 2) {
          return res
            .status(400)
            .json({ message: "Not enough players to continue the tournament" });
        }
